...
public class SoulSpeedEffect extends MagicMobEffect implements ISyncedMobEffect {
    public SoulSpeedEffect(MobEffectCategory mobEffectCategory, int color) {
        super(mobEffectCategory, color);
    }
    public static final float SPEED_PER_LEVEL = .5f;
    ...
    public static boolean doEffect(LivingEntity livingEntity, DamageSource damageSource) {
        ...
        level.playSound(null, livingEntity.getX(), livingEntity.getY(), livingEntity.getZ(), SoundEvents.SOUL_ESCAPE, SoundSource.AMBIENT, 1.0F, .9F + random.nextFloat() * .2f);
        return true;
    }
    ...
    private static void particleCloud(LivingEntity entity) {
        ...
        MagicManager.spawnParticles(entity.level(), ParticleTypes.SOUL, pos.x, pos.y, pos.z, 70, entity.getBbWidth() / 4, entity.getBbHeight() / 5, entity.getBbWidth() / 4, .035, false);
    }
    ...
    public void clientTick(LivingEntity entity, MobEffectInstance instance) {
        ...
        for (int i = 0; i < 2; i++) {
            ...
            entity.level().addParticle(ModParticleHelper.REAPER_FIRE, entity.getRandomX(.4f), entity.getRandomY(), entity.getRandomZ(.4f), motion.x, motion.y, motion.z);
        }
    }
}
