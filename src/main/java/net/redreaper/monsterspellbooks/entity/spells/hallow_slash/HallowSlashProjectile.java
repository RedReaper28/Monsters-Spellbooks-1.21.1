package net.redreaper.monsterspellbooks.entity.spells.hallow_slash;
...
public class HallowSlashProjectile extends AbstractMagicProjectile implements AntiMagicSusceptible {
    private int lifetimeInTicks = 20 * 10;


    public HallowSlashProjectile(EntityType<? extends HallowSlashProjectile> entityType, Level level) {
        super(entityType, level);

        this.setNoGravity(true);
    }

    public HallowSlashProjectile(EntityType<? extends HallowSlashProjectile> entityType, Level level, LivingEntity shooter) {
    ...
    public HallowSlashProjectile(Level level, LivingEntity shooter) {
        this(ModEntities.HALLOW_SLASH_PROJECTILE.get(), level, shooter);
    }
    ...
    public void trailParticles() {
        if (level().isClientSide){
            ...
            for (int i = 0; i < width / step; i++) {
                ...
                level().addParticle(ParticleHelper.WISP, false, x + rotX + dx, y + dy, z + rotZ + dz, dx, dy, dz);
            }
        }
    }
    ...
    public void impactParticles(double x, double y, double z) {
        MagicManager.spawnParticles(level(), ParticleHelper.WISP, x, y, z, 1, 0, 0, 0, 0.1, true);
    }
    ...
    public Optional<Holder<SoundEvent>> getImpactSound() {
        return Optional.of(SoundRegistry.SUNBEAM_IMPACT);
    }
    ...
    protected void onHitEntity(EntityHitResult pResult) {
        ...
        DamageSources.applyDamage(pResult.getEntity(), getDamage(),
                ModSpellRegistry.HALLOW_SLASH.get().getDamageSource(this, getOwner()));
