...
public class ObsidianArrowVolleyEntity extends AbstractMagicProjectile {
    public ObsidianArrowVolleyEntity(EntityType<? extends Projectile> pEntityType, Level pLevel) {
        ...
        if (!level().isClientSide) {
            if (tickCount % delay == 0) {
                ...
                for (int i = 0; i < arrows; i++) {
                    ...
                    SmallObsidianArrowProjectile arrow = new SmallObsidianArrowProjectile(this.level(), this.getOwner());
                    ...
                    MagicManager.spawnParticles(level(), ParticleHelper.ENDER_SPARKS, spawn.x, spawn.y, spawn.z, 2, .1, .1, .1, .05, false);
                }
                level().playSound(null, position().x, position().y, position().z, SoundEvents.HEAVY_CORE_HIT, SoundSource.NEUTRAL, 3.0f, 1.1f + Utils.random.nextFloat() * .3f);
