...
public class PaleThornRenderer extends EntityRenderer<PaleThornVisualEntity> {

    public static final ModelLayerLocation MODEL_LAYER_LOCATION = new ModelLayerLocation(ResourceLocation.fromNamespaceAndPath(MonstersSpellbooks.MOD_ID, "pale_thorn_model"), "main");
    private static final ResourceLocation TEXTURE_CORE = MonstersSpellbooks.id("textures/entity/pale_thorn/core.png");
    private static final ResourceLocation TEXTURE_OVERLAY = MonstersSpellbooks.id("textures/entity/pale_thorn/overlay.png");

    private final ModelPart body;

    public PaleThornRenderer(EntityRendererProvider.Context context) {
        super(context);
    ...
    @Override
    public boolean shouldRender(PaleThornVisualEntity pLivingEntity, Frustum pCamera, double pCamX, double pCamY, double pCamZ) {
        return true;
    }

    @Override
    public void render(PaleThornVisualEntity entity, float yaw, float partialTicks, PoseStack poseStack, MultiBufferSource bufferSource, int light) {
        poseStack.pushPose();
        float lifetime = PaleThornVisualEntity.lifetime;
        float scalar = .25f;
    ...
    @Override
    public ResourceLocation getTextureLocation(PaleThornVisualEntity entity) {
        return TEXTURE_CORE;
    }
}
