...
public class RedstoneExplosionEntity extends AbstractMagicProjectile {
    ...
    private final static Supplier<AbstractSpell> SPELL = ModSpellRegistry.REDSTONE_CHAIN;

    public RedstoneExplosionEntity(EntityType<? extends Projectile> pEntityType, Level pLevel) {
    ...
    public RedstoneExplosionEntity(Level level, Entity owner, Entity initialVictim) {
        this(ModEntities.REDSTONE_CHAIN_LIGHTNING.get(), level);
        this.setOwner(owner);
    ...
    public void doHurt(Entity victim) {
        ...
        MagicManager.spawnParticles(level(), ModParticleHelper.REDSTONE_SPARKS, victim.getX(), victim.getY() + victim.getBbHeight() / 2, victim.getZ(), 10, victim.getBbWidth() / 3, victim.getBbHeight() / 3, victim.getBbWidth() / 3, 0.1, false);
