...
public class CaveSpiderFangProjectile extends AbstractMagicProjectile implements GeoEntity {
    private final AnimatableInstanceCache cache;
    private final RawAnimation idle;

    public CaveSpiderFangProjectile(EntityType<? extends Projectile> pEntityType, Level pLevel) {
        super(pEntityType, pLevel);
        this.cache = GeckoLibUtil.createInstanceCache(this);
        this.idle = RawAnimation.begin().thenLoop("idle");
        this.setNoGravity(true);    }

    public CaveSpiderFangProjectile(Level levelIn, LivingEntity shooter) {
        this(ModEntities.CAVE_SPIDER_FANG.get(), levelIn);
        setOwner(shooter);
    }
    ...
    public Optional<Holder<SoundEvent>> getImpactSound() {
        return Optional.of(BuiltInRegistries.SOUND_EVENT.wrapAsHolder(SoundEvents.SPIDER_HURT));
    }
    ...
    protected void onHitEntity(EntityHitResult entityHitResult) {
        ...
        {
            livingTarget.addEffect(new MobEffectInstance(MobEffects.POISON,300, 1));
        }
        pierceOrDiscard();
    }
    ...
    public void impactParticles(double x, double y, double z) {
        MagicManager.spawnParticles(level(), ParticleHelper.ACID, x, y, z, 1, .1, .1, .1, .25, true);
    }
