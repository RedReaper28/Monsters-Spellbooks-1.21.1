...
public class IceSpiderFangProjectile extends AbstractMagicProjectile implements GeoEntity {
    private final AnimatableInstanceCache cache;
    private final RawAnimation idle;

    public IceSpiderFangProjectile(EntityType<? extends Projectile> pEntityType, Level pLevel) {
        super(pEntityType, pLevel);
        this.cache = GeckoLibUtil.createInstanceCache(this);
        this.idle = RawAnimation.begin().thenLoop("idle");
        this.setNoGravity(true);    }

    public IceSpiderFangProjectile(Level levelIn, LivingEntity shooter) {
        this(ModEntities.ICE_SPIDER_FANG.get(), levelIn);
        setOwner(shooter);
    }
    ...
    protected void onHitEntity(EntityHitResult entityHitResult) {
        ...
        {
            livingTarget.addEffect(new MobEffectInstance(ModMobEffects.SPIRIT_FROSTBITE,300, 3));
        }
        pierceOrDiscard();
    }
    ...
    public void impactParticles(double x, double y, double z) {
        MagicManager.spawnParticles(level(), ParticleHelper.SNOW_DUST, x, y, z, 1, .1, .1, .1, .25, true);
    }
    ...
    public void trailParticles() {
        ...
        for (int i = 0; i < count; i++) {
            ...
            level().addParticle(ParticleHelper.SNOWFLAKE, this.getX() + random.x + p.x, this.getY() + random.y + p.y, this.getZ() + random.z + p.z, random.x, random.y, random.z);
        }
    }
