package net.redreaper.monsterspellbooks.entity.spells.vitaL_blast;
...
public class VitalBlastRenderer extends EntityRenderer<VitalBlastVisualEntity> {
    ...
    private static final ResourceLocation TEXTURE_CORE = MonstersSpellbooks.id("textures/entity/vital_blast/core.png");
    private static final ResourceLocation TEXTURE_OVERLAY = MonstersSpellbooks.id("textures/entity/vital_blast/overlay.png");

    private final ModelPart body;

    public VitalBlastRenderer(EntityRendererProvider.Context context) {
        super(context);
    ...
    @Override
    public boolean shouldRender(VitalBlastVisualEntity pLivingEntity, Frustum pCamera, double pCamX, double pCamY, double pCamZ) {
        return true;
    }

    @Override
    public void render(VitalBlastVisualEntity entity, float yaw, float partialTicks, PoseStack poseStack, MultiBufferSource bufferSource, int light) {
        poseStack.pushPose();
        float lifetime = VitalBlastVisualEntity.lifetime;
        float scalar = .25f;
        ...
        for (float i = 0; i < entity.distance * 4; i += length) {
            ...
            VertexConsumer consumer = bufferSource.getBuffer(RenderHelper.CustomerRenderType.magicNoCull(TEXTURE_OVERLAY));
            {
                ...
                poseStack.scale(expansion, 0.75f, expansion);
            ...
            consumer = bufferSource.getBuffer(RenderType.entityCutoutNoCull(TEXTURE_CORE));
            {
                ...
                poseStack.scale(expansion, 0.50f, expansion);
    ...
    @Override
    public ResourceLocation getTextureLocation(VitalBlastVisualEntity entity) {
        return TEXTURE_CORE;
    }
}
