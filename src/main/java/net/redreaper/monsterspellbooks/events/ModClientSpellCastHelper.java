    ...
    public static void handleClientboundSoulExplosion(Vec3 pos, float radius) {
        MinecraftInstanceHelper.ifPlayerPresent(player -> {
            ...
            for (int i = 0; i < c; i++) {
                ...
                level.addParticle(ParticleTypes.LARGE_SMOKE, x + posOffset.x, y + posOffset.y, z + posOffset.z, vec3.x, vec3.y, vec3.z);
            }
            //Smoke Cloud
            ...
            for (int i = 0; i < cloudDensity; i += 2) {
                ...
                level.addParticle(ModParticleHelper.REAPER_FIRE, true, x + posOffset.x, y + posOffset.y, z + posOffset.z, motion.x, motion.y, motion.z);
            }
            //Sparks
            for (int i = 0; i < cloudDensity; i += 2) {
