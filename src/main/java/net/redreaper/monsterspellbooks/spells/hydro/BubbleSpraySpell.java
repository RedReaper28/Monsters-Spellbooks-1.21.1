...
public class BubbleSpraySpell extends AbstractSpell {
    private final ResourceLocation spellId = ResourceLocation.fromNamespaceAndPath(MonstersSpellbooks.MOD_ID, "bubble_spray");
    ...
    public void onCast(Level world, int spellLevel, LivingEntity entity, CastSource castSource, MagicData playerMagicData) {
        if (playerMagicData.isCasting() && playerMagicData.getCastingSpellId().equals(this.getSpellId())
            ...
            BubbleSprayProjectile bubbledBreathProjectile = new BubbleSprayProjectile(world, entity);
            bubbledBreathProjectile.setPos(entity.position().add(0, entity.getEyeHeight() * .7, 0));
            bubbledBreathProjectile.setDamage(getDamage(spellLevel, entity));
            world.addFreshEntity(bubbledBreathProjectile);

            playerMagicData.setAdditionalCastData(new EntityCastData(bubbledBreathProjectile));
        }
