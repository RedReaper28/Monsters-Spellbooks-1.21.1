...
public class GraveyardFissureSpell extends AbstractSpell {
    ...
    public void onCast(Level world, int spellLevel, LivingEntity entity, CastSource castSource, MagicData playerMagicData) {
        ...
        for (int i = 0; i < getCount(spellLevel, entity); i++) {
            ...
            spawn = new Vec3(spawn.x, getGroundLevel(world, spawn, 16), spawn.z);
            if (!world.getBlockState(BlockPos.containing(spawn).below()).isAir()) {
                int delay = i / 3;
                GraveyardHand hand = new GraveyardHand(world, entity);
                hand.setDamage(getDamage(spellLevel, entity));
                hand.moveTo(spawn.add(0, 0, 0));
                hand.setYRot((entity.getYRot() - 45 + Utils.random.nextIntBetweenInclusive(-60, 60)));
                hand.setXRot(Utils.random.nextIntBetweenInclusive(-60, 60));
                world.addFreshEntity(hand);
            }
        }
